...
//int (*LINK_get_sensor_selection)(void);
...
static const str_map effects[] = {
    { CameraParameters::EFFECT_NONE,       CAMERA_EFFECT_OFF }, // tick
    { CameraParameters::EFFECT_MONO,       CAMERA_EFFECT_MONO }, // tick
...
static const str_map focus_modes[] = {
	{ CameraParameters::FOCUS_MODE_AUTO,     0}, // AF_MODE_AUTO },
      ...
      //,mDebugFps(0)
{
    ...
    if ((pthread_create(&w_thread, NULL, opencamerafd, NULL)) != 0) {
        ALOGE("Camera open thread creation failed");
    }
...
void QualcommCameraHardware::initDefaultParameters()
{
    ...
    if((strcmp(mSensorInfo.name, "vx6953")) &&
        ...
        (strcmp(mSensorInfo.name, "mt9p012")) &&
	...
	mParameters.set("brightness-max", "6");
...
bool QualcommCameraHardware::startCamera()
{
    ...
    *(void **)&LINK_zoom_crop_upscale =
    ...
    //*(void **)&LINK_get_sensor_selection =
    //    ::dlsym(libmmcamera, "get_sensor_selection");
#else
    ...
    if (pthread_join(w_thread, NULL) != 0) {
        ALOGE("Camera open thread exit failed");
        return false;
    }
    mCameraControlFd = camerafd;

    if (mCameraControlFd < 0) {
        ALOGE("startCamera X: %s open failed: %s!",
             MSM_CAMERA_CONTROL,
             strerror(errno));
        return false;
    }
...
static bool native_get_maxzoom(int camfd, void *pZm)
{
    ...
    if (ioctl(camfd, MSM_CAM_IOCTL_CTRL_COMMAND, &ctrlCmd) < 0) {
        ALOGE("native_get_maxzoom: ioctl fd %d error %s",
             camfd,
             strerror(errno));
        return false;
    }
    ALOGD("ctrlCmd.value = %d", *(int32_t *)ctrlCmd.value);
    memcpy(pZoom, (int32_t *)ctrlCmd.value, sizeof(int32_t));

    ALOGV("native_get_maxzoom X");
    return true;
}
*/
...
static bool native_stop_recording(int camfd)
{
    ...
    if ((ret = ioctl(camfd, MSM_CAM_IOCTL_CTRL_COMMAND, &ctrlCmd)) < 0) {
        ALOGE("native_stop_recording: ioctl failed. ioctl return value is %d \n",
        ret);
        return false;
    }
...
bool QualcommCameraHardware::native_jpeg_encode(void)
{
   ...
   model[modelLen] = '\0';
    ...
    if (!LINK_jpeg_encoder_encode(&mDimension,
                                  ...
                                  &mCrop
								  ,exif_data, exif_table_numEntries,jpegPadding/2
								  )) {
    ...
    cam_ctrl_type type, uint16_t length, void *value)
{
    ...
    ALOGD("%s: fd %d, type %d, length %d", __FUNCTION__,
...
void QualcommCameraHardware::runFrameThread(void *data)
{
    ...
    ALOGV("FRAME: loading libqcamera at %p", libhandle);
    if (!libhandle) {
        ALOGE("FATAL ERROR: could not dlopen liboemcamera.so: %s", dlerror());
    }
    if (libhandle)
...
void QualcommCameraHardware::runVideoThread(void *data)
{
    ...
    while(true) {
        ...
        if(vframe != NULL) {
            ...
            ALOGV("Got video frame :  buffer %d base %d ", vframe->buffer, mRecordHeap->mHeap->base());
            ssize_t offset =
                (ssize_t)vframe->buffer - (ssize_t)mRecordHeap->mHeap->base();
            ALOGV("offset = %d , alignsize = %d , offset later = %d", offset, mRecordHeap->mAlignedBufferSize, (offset / mRecordHeap->mAlignedBufferSize));
...
bool QualcommCameraHardware::initPreview()
{
    ...
    ALOGD("initPreview E: preview size=%dx%d videosize = %d x %d", previewWidth, previewHeight, videoWidth, videoHeight );
/*
    if( ( mCurrentTarget == TARGET_MSM7630 ) || (mCurrentTarget == TARGET_QSD8250)) {
        ...
        ALOGV("initPreview : preview size=%dx%d videosize = %d x %d", previewWidth, previewHeight, mDimension.video_width, mDimension.video_height );
    }
*/

    mFrameThreadWaitLock.lock();
    while (mFrameThreadRunning) {
        ALOGV("initPreview: waiting for old frame thread to complete.");
        mFrameThreadWait.wait(mFrameThreadWaitLock);
        ALOGV("initPreview: old frame thread completed.");
    }
    mFrameThreadWaitLock.unlock();

    mInSnapshotModeWaitLock.lock();
    while (mInSnapshotMode) {
        ALOGV("initPreview: waiting for snapshot mode to complete.");
        mInSnapshotModeWait.wait(mInSnapshotModeWaitLock);
        ALOGV("initPreview: snapshot mode completed.");
    }
...
*/
    ...
    if (ret) {
        for (cnt = 0; cnt < kPreviewBufferCount; cnt++) {
            ...
            frames[cnt].path = MSM_FRAME_ENC; //OUTPUT_TYPE_P; // MSM_FRAME_ENC;
			frames[cnt].cropinfo = NULL;
			frames[cnt].croplen = 0;
        }
        ...
        mFrameThreadRunning = !pthread_create(&mFrameThread,
                                              &attr,
                                              frame_thread,
                                              (void*)&(frames[kPreviewBufferCount - 1]));
...
bool QualcommCameraHardware::initRaw(bool initJpegHeap)
{
    ...
    ALOGD("initRaw E: picture size=%dx%d", rawWidth, rawHeight);
    ...
    if (!mRawHeap->initialized()) {
        ALOGE("initRaw X failed with pmem_camera, trying with pmem_adsp");
        mRawHeap =
            new PmemPool("/dev/pmem_adsp",
                         mCameraControlFd,
                         MSM_PMEM_MAINIMG,
...
#warning "Cannot detect multiple release when not dlopen()ing libqcamera!"
	...
	*/
    ...
    if (ioctl(mCameraControlFd, MSM_CAM_IOCTL_CTRL_COMMAND, &ctrlCmd) < 0)
          ALOGE("ioctl CAMERA_EXIT fd %d error %s",
...
{
    ALOGD("in startPreviewInternal : E");
    if(mCameraRunning) {
    ...
    if(native_get_maxzoom(mCameraControlFd, (void *)&mMaxZoom) == true){
        ALOGD("Maximum zoom value is %d", mMaxZoom);
        mParameters.set("zoom-supported", "true");
    } else {
...
status_t QualcommCameraHardware::startPreview()
{
    ...
    //Mutex::Autolock l(&mLock);
	mLock.lock();
	status_t res = startPreviewInternal();
	mLock.unlock();
...
{
    ALOGD("stopPreviewInternal E: %d", mCameraRunning);
    if (mCameraRunning) {
        // Cancel auto focus.
        {
...
*/
	...
	//htc has check for preview running here.  hmmm ...
	...
	// htc has something about LINK_get_sensor_selection, but I think it's just to choose MSM_CAMERA_CONTROL
    ...
    ALOGV("AF: loading libqcamera at %p", libhandle);
    if (!libhandle) {
        ALOGE("FATAL ERROR: could not dlopen liboemcamera.so: %s", dlerror());
        close(mAutoFocusFd);
        mAutoFocusFd = -1;
    ...
    ALOGD("autofocus start (fd %d mode %d)", mAutoFocusFd, afMode);
    status_t err;
    err = mAfLock.tryLock();
    if(err == NO_ERROR)
	{
        {
            Mutex::Autolock cameraRunningLock(&mCameraRunningLock);
            if(mCameraRunning){
                ALOGD("Start autofocus");
    Mutex::Autolock l(&mLock);
                status = native_set_afmode(mAutoFocusFd, afMode);
            }else{
                ALOGD("As Camera preview is not running, autofocus not issued");
                status = false;
            }
        }
        mAfLock.unlock();
        ...
        ALOGD("As Cancel autofocus is in progress, autofocus request "
                "is ignored");
        status = FALSE;
    }

    ALOGD("autofocus done: %d", (int)status);
    close(mAutoFocusFd);
    mAutoFocusFd = -1;
...
*/
    if (mAutoFocusFd < 0) {
        ALOGV("cancelAutoFocusInternal X: not in progress");
        return NO_ERROR;
    }
    ...
    if(err == NO_ERROR) {
        ...
        ALOGD("As AutoFocus is not in progress, so ignore cancel");
        mAfLock.unlock();
    }
    else {
        //AF is in Progess, So cancel it
        ALOGD("AutoFocus Lock busy, so cancel it");
    Mutex::Autolock l(&mLock);
    ...
    ALOGD("cancelAutoFocusInternal X: %d", rc);
    return rc;
}
...
void *auto_focus_thread(void *user)
{
    ...
    if (obj != 0) {
		...
		ALOGW("not starting autofocus: the object went away!");
...
status_t QualcommCameraHardware::autoFocus()
{
	...
	//htc has a check here to see if preview is running. hmmm. so adding my own
	if (!mPreviewInitialized)
	{
        ALOGE("not starting autofocus: mPreviewInitialized=false");
        return UNKNOWN_ERROR;
	}

    if (mCameraControlFd < 0) {
        ALOGE("not starting autofocus: main control fd %d", mCameraControlFd);
        return UNKNOWN_ERROR;
    }

    {
        ...
        if (!mAutoFocusThreadRunning) {
            ...
            if (native_prepare_snapshot(mCameraControlFd) == FALSE) {
               ALOGE("native_prepare_snapshot failed!\n");
               mAutoFocusThreadLock.unlock();
               return UNKNOWN_ERROR;
            }
			*/
...
void QualcommCameraHardware::runSnapshotThread(void *data)
{
    ...
    while (mJpegThreadRunning) {
        ALOGV("runSnapshotThread: waiting for jpeg thread to complete.");
        mJpegThreadWait.wait(mJpegThreadWaitLock);
        ALOGV("runSnapshotThread: jpeg thread completed.");
    }
    mJpegThreadWaitLock.unlock();
	*/
...
status_t QualcommCameraHardware::setParameters(const CameraParameters& params)
{
	...
	//setMeterMode? // 12==CAMERA_SET_PARM_EXPOSURE?
	int32_t meter_mode = 0; // possible values are 0,1,2
	native_set_parm(12, sizeof(meter_mode), (void *)&meter_mode); 

	//setCoordinate? // think is for touch focus. or touch something
	struct {
		int x1,y1;
		int x2,y2; // display width height
		int x3,y3;
		int unknown;
	} umm;
	umm.x2 = mDimension.display_width;
	umm.y2 = mDimension.display_height;
	umm.x3 = mDimension.display_width/2;
	umm.y3 = mDimension.display_height/2;
	umm.x1 = umm.x3/(umm.x2>>2);
	if (umm.x1<3) umm.x1 = 2;
	umm.y1 = umm.y3/(umm.y2>>2);
	if (umm.y1<3) umm.y1 = 2;
	umm.unknown = 0; //?
	native_set_parm(CAMERA_SET_PARM_COORDINATE,sizeof(umm),&umm); 

    if ((rc = setISOValue(params)))  final_rc = rc;

	//setLAMode? 
	int32_t lamode = 0; // off? possible values are 0,1 (smart-contrast)
	native_set_parm(CAMERA_SET_PARM_LA_MODE, sizeof(lamode), (void *)&lamode);
...
status_t QualcommCameraHardware::startRecording()
{
    ...
    if( ret== NO_ERROR) {
        if( ( mCurrentTarget == TARGET_MSM7630 ) || (mCurrentTarget == TARGET_QSD8250))  {
            ALOGV(" in startREcording : calling native_start_recording");
...
/*
    ...
    if( (mCurrentTarget == TARGET_MSM7630 )  || (mCurrentTarget == TARGET_QSD8250)) {
        ...
        ALOGV(" in release recording frame :  heap base %d offset %d buffer %d ", heap->base(), offset, heap->base() + offset );
        int cnt;
...
void QualcommCameraHardware::notifyShutter(common_crop_t *crop = NULL)
{
    mShutterLock.lock();
...
void QualcommCameraHardware::receiveRawSnapshot(){
    ...
    if (mDataCallback && (mMsgEnabled & CAMERA_MSG_COMPRESSED_IMAGE)) {
        ...
        if(native_get_picture(mCameraControlFd, &mCrop) == false) {
            ALOGE("receiveRawSnapshot X: native_get_picture failed!");
            return;
        }
        ...
        if(mRawSnapshotAshmemHeap != NULL){
            ALOGV("receiveRawSnapshot: clearing old mRawSnapShotAshmemHeap.");
            mRawSnapshotAshmemHeap.clear();
        }
        ...
        if(!mRawSnapshotAshmemHeap->initialized()){
            ALOGE("receiveRawSnapshot X: error initializing mRawSnapshotHeap");
            deinitRawSnapshot();
            return;
        }
...
{
	ALOGD("receiveRawPicture: E");

	Mutex::Autolock cbLock(&mCallbackLock);
#if 1
	if (mDataCallback && (mMsgEnabled & CAMERA_MSG_RAW_IMAGE)) {
		ALOGD("receiveRawPicture: CAMERA_MSG_RAW_IMAGE");
		if(native_get_picture(mCameraControlFd, &mCrop) == false) {
			ALOGE("getPicture failed!");
			return;
		}
#if 0
		no crop for now, we are wrabbit hunting
		mCrop.in1_w &= ~1;
		mCrop.in1_h &= ~1;
		mCrop.in2_w &= ~1;
		mCrop.in2_h &= ~1;
		...
		ALOGV("Raw-picture callback was canceled--skipping.");
#else
	notifyShutter();
#endif

	if (mDataCallback && (mMsgEnabled & CAMERA_MSG_COMPRESSED_IMAGE)) {
		ALOGD("receiveRawPicture: CAMERA_MSG_COMPRESSED_IMAGE");
		mJpegSize = 0;
		//mJpegThreadWaitLock.lock();
		if (LINK_jpeg_encoder_init()) {
			...
			if(native_jpeg_encode()) {
				ALOGD("receiveRawPicture: X (success)");
				return;
			}
		...
		ALOGV("JPEG callback is NULL, not encoding image.");
...
{
	ALOGD("receiveJpegPicture: E image (%d uint8_ts out of %d)",
	...
	if (mDataCallback && (mMsgEnabled & CAMERA_MSG_COMPRESSED_IMAGE)) {
		ALOGD("receiveJpegPicture: CAMERA_MSG_COMPRESSED_IMAGE");
           ...
           && height == supportedPreviewSizes[i].height) {
            ...
            if(width == 1280 && height == 720 &&
             ((mCurrentTarget == TARGET_MSM7630) || (mCurrentTarget == TARGET_QSD8250))){
                ALOGD("change preview resolution to 768X432 since recording is in 720p");
...
status_t QualcommCameraHardware::setEffect(const CameraParameters& params)
{
	...
	if (value != NOT_FOUND) {
	    ALOGD("Valid effect value: %s(%i)", (str == NULL) ? "NULL" : str, value);
		mParameters.set(CameraParameters::KEY_EFFECT, str);
		bool ret = native_set_parm(CAMERA_SET_PARM_EFFECT, sizeof(value), (void *)&value);
		return ret ? NO_ERROR : UNKNOWN_ERROR;
	}    
        ...
        if (value != NOT_FOUND) {
            if(!strcmp(sensorType->name, "2mp") && (value != CAMERA_EFFECT_OFF)
               ...
               &&(value != CAMERA_EFFECT_SOLARIZE) && (value != CAMERA_EFFECT_SEPIA)) {
                ALOGE("Special effect parameter is not supported for this sensor");
                return NO_ERROR;
            }
           ...
           || (value == CAMERA_EFFECT_AQUA) || (value == CAMERA_EFFECT_SEPIA))
               ...
               ALOGE("Color Effect value will not be set " \
...
{
    if(!strcmp(sensorType->name, "2mp")) {
        ALOGE("Auto Exposure not supported for this sensor");
        return NO_ERROR;
    }
...
{
    if(!strcmp(sensorType->name, "2mp")) {
        ALOGE("Sharpness not supported for this sensor");
        return NO_ERROR;
    }
...
{
    if(!strcmp(sensorType->name, "2mp")) {
        ALOGE("Contrast not supported for this sensor");
        return NO_ERROR;
    }
...
{
/*     if(!strcmp(sensorType->name, "2mp")) {
        ALOGE("Saturation not supported for this sensor");
        return NO_ERROR;
    }
	const char *str = params.get(CameraParameters::KEY_EFFECT);
    int32_t value = attr_lookup(effects, sizeof(effects) / sizeof(str_map), str);
	...
	return ret ? NO_ERROR : UNKNOWN_ERROR;
/*    } else {
	ALOGE(" Saturation value will not be set " \
...
status_t QualcommCameraHardware::setBrightness(const CameraParameters& params) {
		...
		//{
            ALOGV(" set brightness : %d ", brightness);

            bool ret = native_set_parm(CAMERA_SET_PARM_BRIGHTNESS, sizeof(brightness),
                                       (void *)&brightness);
            return ret ? NO_ERROR : UNKNOWN_ERROR;
        //} else {
        //    return NO_ERROR;
...
/*
	...
	*/
        ...
        return BAD_VALUE;
/*    } else {
            ALOGE("Whitebalance value will not be set " \
...
status_t QualcommCameraHardware::setAntibanding(const CameraParameters& params)
{
    ...
    if(!strcmp(sensorType->name, "2mp") || !strcmp(mSensorInfo.name, "ov5642")) {
        ALOGE("Parameter AntiBanding is not supported for this sensor");
        return NO_ERROR;
    }
    return NO_ERROR;
	*/
...
{
    int32_t zoom_level = (params.getInt("zoom") * 40)/9;
	if (zoom_level < 0)
		zoom_level = 0;
	ALOGI("Set zoom=%d", zoom_level);
	mParameters.set("zoom", zoom_level);
	bool ret = native_set_parm(CAMERA_SET_PARM_ZOOM, sizeof(zoom_level), (void *)&zoom_level);
	return ret ? NO_ERROR : UNKNOWN_ERROR;
}
    ...
    mBuffers(NULL), mName(name)
{
	...
	if (strcmp(name,"snapshot camera")!=0)
		mAlignedBufferSize = clp2(mAlignedBufferSize);
}
    ...
    mCameraControlFd(dup(camera_control_fd))
{
    ...
    ALOGV("%s: duplicating control fd %d --> %d",
         __FUNCTION__,
    ...
    if (pmemHeap->getHeapID() >= 0) {
        ...
        if( (strcmp("postview", mName) != 0) ){
            ...
            for (int cnt = 0; cnt < num_buf; ++cnt) {
                ...
                else*/ if (pmem_type == MSM_PMEM_OUTPUT2) {
...
QualcommCameraHardware::PmemPool::~PmemPool()
{
    ...
    ALOGV("destroying PmemPool %s: closing control fd %d",
         mName,
         mCameraControlFd);
    close(mCameraControlFd);
...
//#define PAD_TO_WORD(x) ((x&1) ? x+1 : x)
	...
	int cbcr_off = (frame_size * 2 / 3);
	pmemBuf.cbcr_off = (cbcr_off+3)&(~3); 

    pmemBuf.active   = vfe_can_write;

    ALOGD("register_buf: camfd = %d, reg = %d buffer = %p",
...
void QualcommCameraHardware::receive_camframetimeout(void) {
    ALOGV("receive_camframetimeout: E");
    Mutex::Autolock l(&mCamframeTimeoutLock);
    ALOGD(" Camframe timed out. Not receiving any frames from camera driver ");
    camframe_timeout_flag = TRUE;
    mNotifyCallback(CAMERA_MSG_ERROR, CAMERA_ERROR_UKNOWN, 0,
                    mCallbackCookie);
    ALOGV("receive_camframetimeout: X");
}
/*
static void receive_camframetimeout_callback(void) {
    sp<QualcommCameraHardware> obj = QualcommCameraHardware::getInstance();
